...
def naive_softmax_loss_and_gradient(
    ...
    outside_vectors_center_vec_prod = np.matmul(outside_vectors, center_word_vec.T)
    softmax_dist = softmax(outside_vectors_center_vec_prod)

    loss = - np.log(softmax_dist[outside_word_idx])

    expected_outside_word = np.matmul(softmax_dist.T, outside_vectors)
    grad_center_vec = expected_outside_word - outside_vectors[outside_word_idx]

    grad_outside_vecs = np.zeros_like(outside_vectors)
    for (i,outside_vec) in enumerate(outside_vectors):
        if i == outside_word_idx:
           grad_outside_vecs[i] = (softmax_dist[outside_word_idx] - 1) * center_word_vec
        else:
            grad_outside_vecs[i] = softmax_dist[i] * center_word_vec
...
def neg_sampling_loss_and_gradient(
    ...
    loss = - np.log(sigmoid(outside_vectors_center_vec_prod[outside_word_idx]))
    grad_center_vec = - (1 - sigmoid(outside_vectors_center_vec_prod[outside_word_idx])) * outside_vectors[outside_word_idx]
    for k in neg_sample_word_indices:
        loss -= np.log(sigmoid(- outside_vectors_center_vec_prod[k]))
        grad_center_vec += (1 - sigmoid(- outside_vectors_center_vec_prod[k])) * outside_vectors[k]

    grad_outside_vecs = np.zeros_like(outside_vectors)
    grad_outside_vecs[outside_word_idx] = - (1 - sigmoid(outside_vectors_center_vec_prod[outside_word_idx])) * center_word_vec
    occurrences_dict = Counter(neg_sample_word_indices)
    for (k, num) in occurrences_dict.items():
        grad_outside_vecs[k] = num * (1 - sigmoid(- outside_vectors_center_vec_prod[k])) * center_word_vec
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    grad_center_vec = np.zeros_like(center_word_vectors[0]).astype('float64')

    center_word_vec = center_word_vectors[word2ind[current_center_word]]

    for word in outside_words:
        loss_tmp, grad_center_vec_tmp, grad_outside_vectors_tmp = word2vec_loss_and_gradient(center_word_vec, word2ind[word], outside_vectors, dataset)
        loss += loss_tmp
        grad_center_vec += grad_center_vec_tmp
        grad_outside_vectors += grad_outside_vectors_tmp

    grad_center_vecs[word2ind[current_center_word]] = grad_center_vec
